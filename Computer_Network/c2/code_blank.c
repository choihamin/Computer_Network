...
/*
	dst_ip에 도달할 수 있는지 여부를 routing table에서 확인해야 함.
	이에 따라, 라우팅 엔트리에서 dst_ip로 포워딩 가능한 라우터의 주소(여기서는 ptr)를 반환해줌.
		1. 이 함수는 가장 처음 찾는 엔트리를 longest matching되는 엔트리라고 함.
		2. strncmp 함수에 의해, matching되는 가장 최초의 라우터 주소를 반환함.

	=> 목적지까지 패킷을 전달하기 위해 포워딩 가능한 다음 라우터의 주소 (= next hop)을 찾기 위해 본 함수가 필요합니다.
...
int my_ip_output(struct my_ip_header my_iph, struct my_ip_routing_entry* rentry) {
	...
	*((unsigned short*)buffer_ptr) = htons(my_iph.dest_len);
	...
	memcpy(buffer_ptr, (unsigned char*)my_iph.dest, my_iph.dest_len);
	buffer_ptr += my_iph.dest_len;

	*((unsigned short*)buffer_ptr) = htons(my_iph.source_len);
	buffer_ptr += sizeof(unsigned short);

	memcpy(buffer_ptr, (unsigned char*)my_iph.source, my_iph.source_len);
	buffer_ptr += my_iph.source_len;

	*((unsigned char*)buffer_ptr) = my_iph.ttl;
...
void environment_setup() {
	...
	sscanf("08:00:27:e1:fa:b4", "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", &mac[0], &mac[1], &mac[2], &mac[3], &mac[4], &mac[5]);
	add_arp_cache_entry("enp0s3", "Dongdaemungu 1", mac);
	sscanf("08:00:27:05:1f:76", "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", &mac[0], &mac[1], &mac[2], &mac[3], &mac[4], &mac[5]);
